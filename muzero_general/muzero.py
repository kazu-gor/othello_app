...
class MuZero:
    ...
    def application_match(self, render = True, board=None, observation=None):
        ...
        #         self.config.train_on_gpu
        #         + self.config.num_workers * self.config.selfplay_on_gpu
        #         + False * self.config.selfplay_on_gpu
        #         + self.config.use_last_model_value * self.config.reanalyse_on_gpu
        #     )
        #     if 1 < num_gpus_per_worker:
        #         num_gpus_per_worker = math.floor(num_gpus_per_worker)
        ...
        game_history, next_action = ray.get(self_play_worker.play_game_beta.remote(
        ...
        # self.replay_buffer_worker.save_game.remote(game_history, self.shared_storage_worker)
        # self_play_worker.close_game.remote()
        
        # if len(self.config.players) == 1:
        #     result = numpy.mean(sum(game_history.reward_history))
        # else:
        #     result = numpy.mean(
        #         [
        #             sum(
        ...
        #                 if game_history.to_play_history[i - 1] == 0
        #             )
        #         ]
        #     )
...
if __name__ == "__main__":
    ...
    #         choice = input("Invalid input, enter a number listed above: ")
        ...
        #         muzero.application_match(render=True, observation = observation)
        #         break
        #     else:
        #         break
    board = numpy.zeros((8, 8), dtype="int32")
