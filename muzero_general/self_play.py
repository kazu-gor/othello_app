...
class SelfPlay:
    ...
    def __init__(self, initial_checkpoint, Game, config, seed):
        self.config = config
        self.game = Game(seed)
        self.game.reset_manual(self.config.board)
    ...
    def play_game_beta(
        self, temperature, temperature_threshold, render, observation
    ):
        ...
        if render:
            print("Render in play_game in self_play.py")
            self.game.render()
        ...
        with torch.no_grad():
            ...
            if not self.game.legal_actions():
                return None, -1
    ...
    def play_game(
        ...
        with torch.no_grad():
            print("torch no grad in play_game")
            while (
                ...
                print("in while")
                if not self.game.legal_actions():
                    observation, reward, done = self.game.pass_step()
                    continue
